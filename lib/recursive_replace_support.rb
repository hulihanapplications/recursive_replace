module RecursiveReplaceSupport
  INDENTATION = "  "  
  
  def included(klass)
    klass.send(:extend, RecursiveReplaceSupport::Classmethods)
  end
  
  module ClassMethods        
    def replace(original, replacement, options = {}) # replace text in either a directory(recursively) or single file   
    ...
    def replace_in_file(original, replacement, options = {}) # do the actual replacing in a single file
    ...
    def print_path_with_indent(path) # print pretty path
      indent = String.new
      path.split(File::SEPARATOR).each do 
        indent << ::RecursiveReplaceSupport::INDENTATION unless File.dirname(path) == "."
      end
    ...
    def files_inside(path) # get the working directory to start in
